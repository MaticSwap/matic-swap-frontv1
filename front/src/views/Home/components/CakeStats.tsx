...
const CakeStats = () => {
  ...
  const marketCap = mswapPrice.times(circSupply)

  let mswapPerBlock = 0
  if (farms && farms[0] && farms[0].mswapPerBlock) {
    mswapPerBlock = new BigNumber(farms[0].mswapPerBlock).div(new BigNumber(10).pow(18)).toNumber()
  }
    ...
    <StyledCakeStats>
      <CardBody>
        <Heading size="lg" mb="24px">
          {TranslateString(534, 'Mswap Stats')}
        </Heading>
        <RowHighlighted>
          <Text fontSize="14px">
            <strong>
        ...
        <Row>
          <Text fontSize="14px">
            {TranslateString(538, 'MSWAP eaten by Maticswaps')}{' '}
        ...
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New MSWAP/block')}</Text>
