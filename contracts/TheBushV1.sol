...
// added functionnalities by the Matic's devs
//
...
contract TheBushV1 is Ownable {
    ...
    // The block number when MSWAP mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 mswapReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake MSWAP tokens to TheBushV1
    ...
    // Withdraw MSWAP tokens from STAKING.
